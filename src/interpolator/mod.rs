    ...
    TChannelId: Copy + std::cmp::Eq + std::hash::Hash,
{
    ...
    fn get_interpolated_sample_with_aliasing_filter(
        ...
        //---------------------- bad
        // relative_speed     window_size_original_speed  filter_window_size
        // 1.0                2                           2
        // 1.1                2.2                         2
        // 2.0                4.0                         2
        // 2.2                4.4                         4.4
        ...
        let window_size = relative_speed;
        while window_size > 1.0 {
            index_offset *= 2;
            window_size = window_size / 2.0;
        }

        // relative_speed     window_size        index_offset          sample_ratio
        // 1.0                1                  1.0                   1.0
        // 1.1                2                  0.55                  0.55
        // 2.0                2                  1.0                   1.0
        // 2.2                4                  0.55                  1.1

        let mut transform = Vec::with_capacity(index_offset);
        let index_offset = window_size * (index_offset as f32 / 2.0);
    }

    fn _get_interpolated_sample_with_aliasing_filter_bad(
        &self,
        channel_id: TChannelId,
        index: f32,
        relative_speed: f32,
        ...
        /*
        let maximum_wavelength = 2.0 * relative_speed;
        let required_window_size = (maximum_wavelength * 2.0).ceil() as usize;
        // Round up to next power of 2 (required for FFT)
        let window_size = required_window_size.next_power_of_two();
        */

        // The window size is such that all frequencies are stripped, and the DC bias is the result
        let window_size = 2.0.pow(relative_speed.floor()) as usize;
        while (window_size as f32) / relative_speed > 1.0 {
            window_size *= 2;
        }

        // The window size, in samples, in the original signal. This is long enough to capture the highest frequency
        // that will be present in the anti-aliased signal. The DC bias will be the result, all frequencies are stripped

        // relative_speed    highest wavelength allowed    window size (in original signal)    sample ratio
        // 1.0               2.0                           1.0 (-0.5, +0.5)                    0.5
        // 1.1               2.2                           1.1 (-0.6, +0.6)                    0.55
        // 2.0               4.0                           2.0 (-0.5, +0.5)                    2.0
        // 2.2               4.4                           2.2

        let window_size_f32 = window_size as f32;
        let sample_ratio = relative_speed / window_size_f32;
        let start_index = index - (sample_ratio * window_size_f32 / 2.0);
    }
}
